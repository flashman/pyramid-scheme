...
**Buy-in flow (manual):** `POST /api/buy-in` returns 503 while Stripe is disabled. The authenticated buy-in dialog shows a QR code + a deterministic 5-emoji offering code derived from the username. The player pays externally (e.g. Venmo) and includes the code in the payment note. The admin then confirms it via `POST /api/admin/confirm-buyin {username}` (gated by `User.is_admin`; user 1 is the sole admin, seeded by migration 0003). **Do NOT hand-edit `GameState.bought = True` in the DB** — that grants scrolls but skips the entire upline payout. The endpoint calls `_apply_confirmed_buyin()` in `routers/payments.py`, which is the shared apply path: it marks the buyer bought, grants scrolls, walks the upline via `run_buyin_chain()`, writes ledger rows, and pushes WS events. The same function is the blueprint the Stripe webhook will call once wired (pass `source="stripe"`). The endpoint refuses an already-bought user unless `allow_rebuy=true`, because each confirm pays the chain again.
