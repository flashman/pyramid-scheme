...
- Collision (decided in Task 9 look-dev): rocks, stacks, cliffs, boulders, the wreck and the signal rock are solid. A pure `coast.js` lays them out once for both the scene and the sim; the hull is a capsule. A slow contact scrapes; a hard hit (closing > 3.1 m/s ≈ 6 kn) wrecks her — she sinks over 7 s, then you wash up at the Delta with the Letter kept. (The spec's "no capsizing or failing" is overridden by this decision.)
- Open sea (decided in the Task 15 playthrough): no side walls, no front wall at Crete, no bow-turning and no wind veer — sail off course, around the island or to the horizon; the storm (full by `STORM.offFull` m off the line) is the only deterrent. Crete's shore is an ellipse collider (`coast.js` `creteShoreContact`) with the same scrape/wreck rules as rock; the Delta shore behind the start stays a soft wall.
- Calm bay (decided in Task 9 look-dev): `SHELTER` in constants.js (inner 260 m / outer 520 m around `CRETE_BAY`, amplitude ×0.15) is passed to `glslWaves()` and every buoyancy `heightAt()`, so GPU and physics agree; `stormTarget` falls to `STORM.bay` (0.08) inside it. The beach sand stands ~1 m above sea level so the swell no longer washes over it.
- Haul-up (decided in Task 9 look-dev): once landed the downline (8–16 haulers; hired men make up the numbers) is unchained and sent over the side with two bow ropes — the rowers blink off their benches (`BEACH.ashore` s), then heave her `BEACH.haul` m up the rollers in `BEACH.heaves` brace-and-pull heaves (`haulProgress(t)` in voyage.js, read by ship.js's haul gang), her keel resting on `beachY()`. Events `haul_ropes`, `heave {id}`, `hauled` are narrated; the overseer's whip calls each heave.
...
4. Steer hard off course: a "strayed" line logs; the storm thickens; nothing turns you back — you can sail around Crete or out to the horizon (Crete's shore is solid).
...
9b. Ram a rock at speed: the ship sinks, the wreck line logs, and you wash up in the Delta with the Letter kept. Drift slowly into rock: it only scrapes.
