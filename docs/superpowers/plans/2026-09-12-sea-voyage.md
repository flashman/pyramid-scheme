...
- Haul-up (decided in Task 9 look-dev): once landed the crew drags the ship `BEACH.haul` m up the rollers over `BEACH.haulTime` s, her keel resting on `beachY()`. **Open (user, revisit later):** the haul reads as mysterious — give it dialogue/narration as the crew grabs the ropes and hauls.
