...
- Haul-up (decided in Task 9 look-dev): once landed the Shipmaster's hired men go over the side with two bow ropes (`BEACH.ashore` s), then heave her `BEACH.haul` m up the rollers in `BEACH.heaves` brace-and-pull heaves (`haulProgress(t)` in voyage.js, read by ship.js's haul gang), her keel resting on `beachY()`. Events `haul_ropes`, `heave {id}`, `hauled` are narrated; the overseer's whip calls each heave. The downline stays chained aboard.
